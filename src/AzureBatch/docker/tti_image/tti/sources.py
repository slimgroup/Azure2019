...
class PointSource(SparseTimeFunction):
    ...
    def resample(self, dt=None, num=None, rtol=1e-5, order=3):
        ...
        new_obj = PointSource(name=self.name, grid=self.grid, npoint=self._npoint,
                              time_range=new_time_range, coordinates=self.coordinates)

        for i in range(self._npoint):
            if self.data[:, i].size > 0:
                tck = interpolate.splrep(self._time_range.time_values,
                                         self.data[:, i], k=order)
                new_obj.data[:, i] = interpolate.splev(new_time_range.time_values, tck)
